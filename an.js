with(document) {
    ...
    function s(n) {
        m = getElementsByClassName('ms');
        d = getElementsByClassName('d');
        if (n > m.length) j = 1;
        if (n < 1) j = m.length
    ...
    with(Math) {
        ...
        x = setInterval(function() {
            ...
            let months = floor(distance / (3600 * 24e3 * 28))
            let weeks = floor(distance / (3600 * 24e3 * 7));
            let y = floor(distance / (3600 * 24e3 * 28 * 12))
            let days = floor(distance / (3600 * 24e3));
            let hours = floor((distance % (3600 * 24e3)) / 36e5);
            let minutes = floor((distance % 36e5) / 6e4);
            let seconds = floor((distance % 6e4) / 1e3)
        ...
        setInterval(function() {
            ...
            let d = n - birthday;
            let mo = round(d / (3600 * 24e3 * 28));
            let w = floor(d / (3600 * 24e3 * 7))
            let da = floor(d / (3600 * 24e3));
            let h = floor((d % (3600 * 24e3)) / 36e5)
            let m = floor((d % 36e5) / 6e4);
            let s = floor((d % 6e4) / 1e3)
...
window.onload = function nhay() {
    let n = name.substring(count, name.length);
    let o = name.substring(0, count)
    namea.firstChild.nodeValue = n + ' ' + o;
    nameb.firstChild.nodeValue = o;
    namec.firstChild.nodeValue = n;
    count++
    ...
    if (anh < 2) {
        anbot[anh].style.display = 'none';
        anbot[anh + 1].style.display = 'block'
    } else {
        anbot[anh].style.display = 'none';
        anbot[anh - 2].style.display = 'block'
    }
    if (lap < 2) {
